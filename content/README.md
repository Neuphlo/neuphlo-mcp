...
Required fields are `id`, `type`, `area`, `title`, `status`, `owner`, `created`, and `updated`. `sensitivity` and `tags` are useful optional fields. New records are placed below `areas/<area>/`; standard types use their matching plural folder and custom types use a folder named after the type.
